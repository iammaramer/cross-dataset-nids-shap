...
OUTPUT_DIR = r"D:\thesis_implementation\results\features_analysis"
...
def load_model(exp, model_name):
    fname = os.path.join(MODELS_DIR, f"{exp}_{model_name}.pkl")
...
def run_stability_analysis(exp, model_name, X_test, feature_names):
    print(f"\n  {exp} | {model_name} — running {N_RUNS} SHAP runs...")

    try:
        model = load_model(exp, model_name)
...
def build_stability_summary(all_runs, feature_names, exp, model_name):
    run_records = [r[0] for r in all_runs]
    imp_dfs     = [r[1] for r in all_runs]

    rankings_df = pd.DataFrame(run_records)

    # Count top-K appearances per feature
    all_top_features = []
    ...
    stability_df = (
        pd.DataFrame(counts.items(), columns=["Feature", "Top10_Appearances"])
...
def plot_stability_bar(merged_df, exp, model_name, output_dir):
    ...
    ax.set_title(f"SHAP Stability: {exp} | {model_name}\n(Top 20 features, {N_RUNS} runs)")
    ax.grid(axis="x", linestyle="--", alpha=0.4)

    plt.tight_layout()
    out_path = os.path.join(output_dir, f"shap_stability_{exp}_{model_name}.png")
...
def main():
    print("Stage 4 — SHAP Stability Re-run (All Models, C1 & C2)")
    print("=" * 60)
    ...
    test_splits = {
        "C1": (X17_test, y17_test),
        "C2": (X18_test, y18_test),
    }

    all_stability_rows = []

    for exp, (X_test, _) in test_splits.items():
        print(f"\n{'='*60}")
        print(f"Experiment: {exp} ({EXPERIMENTS[exp]})")
        print(f"{'='*60}")

        for model_name in MODELS:
            all_runs = run_stability_analysis(exp, model_name, X_test, feature_names)

            if all_runs is None:
                continue

            rankings_df, stability_df, merged_df = build_stability_summary(
                all_runs, feature_names, exp, model_name
            )
            ...
            plot_stability_bar(merged_df, exp, model_name, OUTPUT_DIR)

            top_stable = stability_df[
                stability_df["Top10_Appearances"] == N_RUNS
            ]["Feature"].tolist()

            all_stability_rows.append({
                "Experiment":              exp,
                "Model":                   model_name,
                "Features_Always_Top10":   len(top_stable),
                "Always_Top10_Features":   ", ".join(top_stable)
            })

    if all_stability_rows:
        combined_df  = pd.DataFrame(all_stability_rows)
        combined_csv = os.path.join(OUTPUT_DIR, "shap_stability_combined_summary.csv")
        combined_df.to_csv(combined_csv, index=False)
        print(f"\n✅ Combined stability summary saved:\n{combined_csv}")
        print("\nCombined Summary:")
