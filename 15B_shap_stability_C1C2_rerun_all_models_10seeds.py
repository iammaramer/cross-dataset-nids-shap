...
def run_stability_analysis(exp, model_name, X_test, feature_names):
    ...
    for i, seed in enumerate(SEEDS):
        ...
        run_record = {"Run": i + 6, "Seed": seed}  # Run numbers continue from 6
        for rank, feat in enumerate(top_k, 1):
            run_record[f"Rank_{rank}"] = feat

        all_runs.append((run_record, imp_df))
        print(f"    Run {i+6} (seed={seed}) — Top 3: {top_k[:3]}")
...
def build_stability_summary(all_runs, feature_names, exp, model_name):
    ...
    merged = imp_dfs[0][["Feature", "MeanSHAP"]].rename(columns={"MeanSHAP": "Run6"})
    for i, df in enumerate(imp_dfs[1:], 7):
        merged = merged.merge(
...
def main():
    print("SHAP Stability — Runs 6 to 15 (10 New Seeds)")
    print("=" * 60)
    ...
    for exp, X_test in test_splits.items():
        print(f"\n{'='*60}")
    ...
    if all_stability_rows:
        ...
        print(f"\n✅ Runs 6–15 combined summary saved:\n{combined_csv}")
        print("\nSummary:")
