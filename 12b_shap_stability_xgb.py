...
spearman_csv = os.path.join(RESULTS_DIR, "shap_spearman_XGB_C1C4.csv")
spearman.to_csv(spearman_csv)
print(f"  ✅ Spearman matrix saved → {spearman_csv}")

print("\nSpearman rank correlation matrix (XGBoost SHAP ranks):")
...
ax.set_title("Spearman rank correlation of XGBoost SHAP feature rankings (C1–C4)")
fig.colorbar(cax, ax=ax, label="Spearman ρ")

plt.tight_layout()
heatmap_path = os.path.join(RESULTS_DIR, "shap_spearman_matrix_XGB_C1C4.png")
...
jac_txt = os.path.join(RESULTS_DIR, "shap_jaccard_XGB_C1C4.txt")
with open(jac_txt, "w") as f:
    f.write("Jaccard similarity of top-K XGBoost SHAP features (C1–C4)\n")
    f.write("===================================================\n\n")
    for (a, b) in pairs:
...
jac_csv = os.path.join(RESULTS_DIR, "shap_jaccard_XGB_C1C4.csv")
...
print("\nStage 5 — SHAP stability metrics (XGBoost) complete.")
